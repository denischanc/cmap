...
static void add_include(const char * out_h_name)
{
  ...
  {
    cmap_part_add_include(cmap_file_util_basename(out_h_name), (1 == 1));
    cmap_string_append(cmap_part_includes(), "\n");
  }
  else cmap_part_add_include("cmap-ext.h", (1 == 0));
...
static char generate_c(const char * out_name)
{
  ...
  if(cmap_config_is_add_main()) cmap_build_main_impl(cmap_part_main());

  return cmap_file_util_to_file(out_name, "\n%s\n%s",
    *cmap_part_includes(), *cmap_part_main());
}
...
static char generate_h(const char * out_name)
{
  ...
  char ret = cmap_file_util_to_file(out_name,
    ...
    "\"cmap-ext.h\"" : "<cmap/cmap-ext.h>", *cmap_part_headers());
...
int cmap_build_main(int argc, char * argv[])
{
  ...
  char ok = (1 == 1);
  if(!cmap_do_parse(argv[1])) ok = (1 == 0);
