...
#define HANDLER_IMPL(type, name) \
  if(cmap_op_handler_##type##_##name.op(&map_dst, map_l, map_r, proc_ctx)) \
    return map_dst;
...
#define SELF_HANDLER_IMPL(type, name) \
  if(cmap_op_handler_##type##_##name.op(map_dst, map_src)) return;
...
#define UNIQUE_HANDLER_IMPL(type, name) \
  if(cmap_op_handler_##type##_##name.op(map)) return;

#define UNIQUE_IMPL(name, OP) \
void cmap_op_##name(CMAP_MAP * map) \
{ \
