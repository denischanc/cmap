...
typedef struct SCANNER_NODE_s SCANNER_NODE;
...
typedef struct
{
  void (*$$)(CMAP_MAP * definitions, char * impl, SCANNER_NODE ** chain);
  ...
  CMAP_MAP * (*map)(const char * aisle);
  CMAP_MAP * (*map_args)(CMAP_LIST * args, const char * aisle);

  CMAP_MAP * (*list)(const char * aisle);
  ...
  CMAP_MAP * (*process)(CMAP_MAP * map, const char * fn_name,
    CMAP_LIST * args);
